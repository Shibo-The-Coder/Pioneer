...
<div id='admin_dashboard' class='admin_container'>
    ...
    <th>Due Date</th>
            ...
            foreach ($collabs as $post) {
                $postname = get_table_posts_byID($post['postID'])[0]['title'];
                echo "<tr><td>".admin_make_anchor("posts/edit/".$post['postID'], $postname)."</td><td>".$post['notes']."</td><td>".$post['dueDate']."</td></tr>";
            }
            echo "</table>";
          } ?>
    </div>
